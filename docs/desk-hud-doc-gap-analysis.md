...
- Method: read of `pc/`, `tools/overwatch-statusline.sh`, `claude_usage_bridge.py`,
...
`CONFIG_OVERWATCH_WIFI_MODE` is `default n` (`firmware/Kconfig:16`), and
...
preserves an existing `statusLine.command` into `~/.overwatch/statusline-chain`, and
`tools/overwatch-statusline.sh:49-104` delegates to it on every render. The
...
drops the `statusLine` key, Overwatch goes quiet until the user reinstalls. The daemon
...
Not true of what is *written to disk*. `tools/overwatch-statusline.sh:11,28` captures the
entire payload verbatim to `~/.overwatch/statusline.json`. Per the redaction note in
...
`~/.overwatch` directory at install time. (b) and (c) are compatible and neither touches
the render path.
...
bind a socket anywhere; the shim writes `~/.overwatch/statusline.json.tmp` and `mv -f`s it
into place (`tools/overwatch-statusline.sh:28-29`), and the daemon polls that file every
...
§7 directs descriptive compatibility naming; `README.md:5` leads with "Overwatch" as the
...
| # | Item | Resolution |
|---|---|---|
| 1 | Zero-credential footprint | **Confirmed, doc amended.** Enforced at compile time (`CONFIG_OVERWATCH_WIFI_MODE` default n); the standalone WiFi build is now named as a second mode the document omitted. |
| 2 | Zero outbound AI polling | **Confirmed**, and now trivially so: the extension that observed page responses was removed 2026-08-28, so nothing reaches the network on Overwatch's behalf at all. |
