...
| **What is supported** | Claude Code in a terminal or IDE extension: everything. Codex CLI: everything, on its own page. Claude Desktop alone: a five-hour countdown from Desktop's own record (a rate instead, if that record is not yet known), a weekly countdown once one has been learned, and no activity light. claude.ai in a browser: nothing. |
