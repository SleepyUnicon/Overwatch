...
@anthropic-ai/claude-code`); an OVERWATCH on a USB cable.

Download `overwatch-windows-latest` from the latest CI run's artifacts (Actions ->
the run -> Artifacts), or the release's `overwatch-windows-x86_64.zip` (unzip it; run `overwatch\overwatch.exe`).
...
| 3 | `.\overwatch.exe` | The disclosure, then six steps ending `[6/6] Background service ... running`. No stack trace. Step `[5/6] USB driver` either says `already installed` with no prompt, or raises **one** Windows permission prompt and then says `installed`. **Run this NOT as administrator** -- that is what a customer's double-click does, and the permission prompt is the one path they all take. `this build carries no driver` means the build did not include `vendor/ch341ser`: a release blocker, not a test failure. |
| 4 | `.\overwatch.exe status` | `Bridge registered as a Scheduled Task`, `Claude Code <version>`, `Activity hooks installed (10/10 events)`, `Desktop usage cache parsed, reading N min old`. **If it says `looked at ...`, step 2's path is not the one it checked -- report both.** |
| 5 | Open a terminal, run `claude`, ask it something, wait for the reply. Then `.\overwatch.exe status` again. | `Usage data fresh`, `1 live session`. |
| 6 | `.\overwatch.exe status --wire` | One JSON line with `session_pct`, `weekly_pct`, `provider":"claude"`, `src":"cli"`, `state`. |
...
| 10 | Unplug and replug the board. | Panel back within a minute, no reboot loop, no reset of the board each time (`%USERPROFILE%\.overwatch\bridge.log` says `answered; not resetting it`). |
| 10a | **The ROM loader, which nothing else here covers.** Elevated is not needed. `schtasks /end /tn "Overwatch bridge"`, then `taskkill /f /im overwatch.exe`, then `& "$env:USERPROFILE\.overwatch\bin\overwatch.exe" -m pc.efuse_probe --port COMn`, then `schtasks /run /tn "Overwatch bridge"`. | `Connecting.....`, `Detecting chip type... ESP32`, `flash_encryption disabled`, **in about a second**. This is the one operation a USB firmware update needs and every other step avoids: Windows has only esptool's sequential `ClassicReset`, and on this board's cross-coupled circuit that releases EN before IO0 goes low. A machine that takes ~26 s and fails cannot install firmware at all -- see `docs/open-bugs/windows-usb-ota.md`. Measured 2026-09-08: 13/13 in ~1 s on the desk here, and a customer's machine failing every time. |
| 11 | `.\overwatch.exe uninstall` | Status line and hooks gone from `%USERPROFILE%\.claude\settings.json`, task gone (`schtasks /query /tn "Overwatch bridge"` fails), `%USERPROFILE%\.overwatch\bin` gone a moment after the window closes. |
