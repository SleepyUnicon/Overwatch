...
- **The self-test needs a generous timeout.** 60 s looked ample for
  `overwatch --version` (about 2 s of CPU). Measured at 97 s on a machine under
...
| | |
|---|---|
| One release carries both | `tools/release.sh` tags `v<version.h>` and attaches `overwatch-fw.bin` + `manifest.json`; publishing fires `release-binaries.yml`, which builds the four PyInstaller binaries **from the same tag** and attaches them. |
...
- `firmware/src/version.h` keeps `OVERWATCH_FW_VERSION`; move `PROTO_VERSION` there
  from `proto.c:20` so one header answers both questions.
- New `tests/ci/check_versions.sh`: greps both files, fails if
  `RELEASE_VERSION != OVERWATCH_FW_VERSION` or the two `PROTO_VERSION`s differ.
...
|---|---|---|
| `welcome.v < board PROTO_VERSION` | The computer's app is older than the firmware | Board: persistent settings notice "The Overwatch app on your computer is out of date", plus the update badge. Usage keeps flowing — that path is proto-stable. |
...
- `apply(blob) -> (ok, message)`:
  1. write `~/.overwatch/bin/overwatch.new` (same directory, so the replace is atomic)
  2. `chmod 0755`
  3. **run `overwatch.new --version` and require it to print the expected version.**
  ...
  5. `os.replace(overwatch.new, installed_bin())`
  6. restart (§4.3)
...
| | From the CLI (`overwatch update`) | From inside the daemon |
|---|---|---|
| macOS | `launchctl kickstart -k gui/$UID/com.overwatch.bridge` | `sys.exit(0)` — KeepAlive brings it back within `ThrottleInterval` (10 s) |
| Linux | `systemctl --user restart overwatch-bridge.service` | `sys.exit(0)` — `Restart=always`, `RestartSec=10` |
...
- `overwatch update` — explicit, prints what it will do, applies, restarts.
- `overwatch --version` — needed by 4.2 step 3 anyway; also the first thing anyone
  asks in support.
- `overwatch status` — gains an "App" line: current version, and "update available
...
- Daemon: checks 60 s after the first successful board connect (not at process
  ...
  State in `~/.overwatch/update.json`: `{"last_check", "available", "declined"}`.
- Opt-out that we honour regardless of `daemon.auto`: `OVERWATCH_NO_AUTO_UPDATE=1`
  or a `~/.overwatch/no-auto-update` file.

### 4.5 Signing (D6)

Manifest v2 is signed with a **separate** P-256 release key — not
`~/.overwatch/ota_signing_key_p256.pem`, which is MCUboot's; mixing key purposes is
...
4. On `ota_flash`, if the daemon is behind: write
   `~/.overwatch/pending_fw.json` = `{"version": "0.6.0"}`, self-update, restart.
...
- keep the existing artifact greps (`/api/oauth/usage`, `refresh_token`,
  `claude-code/`, `CONFIG_OVERWATCH_WIFI_MODE=y`)
...
held `overwatch.exe` long enough to matter, so uninstall's
...
2. **The fix killed the uninstaller.** `taskkill /f /im overwatch.exe` as a last
...
| `tests/ci/check_install.sh` | new `update` scenario: install an old build, point `OVERWATCH_OTA_DIR` at a local feed, run `overwatch update`, assert the binary was replaced, `--version` moved, and the service is still registered |
| `release-binaries.yml` | each built binary's `--version` matches the tag |

`OVERWATCH_OTA_DIR` already exists for exactly this kind of local-feed testing and
