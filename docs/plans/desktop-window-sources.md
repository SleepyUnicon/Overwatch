...
- **Never log a buffer.** No byte ranges, byte slices, decoded values or surrounding bytes in any log line, exception message or stderr print, at any level. `README.md:90` promises customers that `~/.overwatch/bridge.log` holds nothing secret; that promise is load-bearing.
...
# Overwatch externalises any value over 65536 bytes to a sibling blob file,
...
README.md:90 tells customers to email support the tail of ~/.overwatch/bridge.log
...
- [ ] `overwatch status` still reports the bridge running and the board fed, on the real machine, with the daemon rebuilt from this branch.
