"""Install the Overwatch execution-state hooks into a user's Claude Code settings.
...
INSTALLED_MARKER_PATH = "~/.overwatch/hooks-installed-commands"
...
HOOK_EVENTS = (
    ...
    # lock -- see tools/overwatch-hook.sh.
    ("SubagentStart", "*"),
    ("SubagentStop", "*"),
)
...
def _ours(command: str, expected: set, marker: set) -> bool:
    ...
    survives a marker file that was lost with the rest of ~/.overwatch.
...
def install(settings_path: str, shim_path: str) -> str:
    ...
    if added == 0 and repointed == 0:
        return "Overwatch state hooks already installed."
    if added == 0:
        return f"Overwatch state hooks repointed at the new path ({repointed})."
    if repointed:
        return (f"Overwatch state hooks installed ({added} events,"
...
def uninstall(settings_path: str, shim_path: str = None) -> str:
    ...
    if not isinstance(hooks, dict):
        _remove_marker()
        return "No Overwatch state hooks to remove."
    ...
    if removed == 0:
        return "No Overwatch state hooks to remove."
    return f"Overwatch state hooks removed ({removed})."
