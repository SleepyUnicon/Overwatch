...
Blob format (header fields little-endian, pixels big-endian unless
...
def encode(frames565, fps, big_endian):
    ...
    blob += struct.pack("<HHBBH", CANVAS_W, CANVAS_H, fps,
                        1 if big_endian else 0, len(frames565))
    per_frame = []
    for idx, (frame, prev) in enumerate(
            zip(frames565, [None] + list(frames565[:-1]))):
        rects = ([(0, 0, CANVAS_W, CANVAS_H)] if prev is None
                 else changed_rects(frame != prev))
        rec = bytearray([len(rects)])
        for x, y, w, h in rects:
            payload = packbits16(frame[y:y + h, x:x + w].ravel())
            rec += struct.pack("<HHHHI", x, y, w, h, len(payload))
            rec += payload
        blob += rec
...
def emit_header(path, blob, fps, nframes):
    lines = [
        "/* Generated by tools/encode_bootanim.py -- do not edit.",
        f" * {nframes} frames @ {fps} fps, {len(blob)} bytes. */",
        "#pragma once",
        "#include <stdint.h>",
        "",
        f"static const uint8_t bootanim_blob[{len(blob)}] = {{",
    ]
...
def main():
    ...
    print(f"blob: {len(blob):,} bytes total")
    ...
    if args.out:
        emit_header(args.out, blob, args.fps, len(frames565))
