...
#
# There are two, they live only in ~/.overwatch, and neither can be regenerated:
#
...
SRC="${OVERWATCH_HOME:-$HOME/.overwatch}"
STAMP=$(date +%Y-%m-%d)
DEST="${1:-$HOME/overwatch-key-backup-$STAMP}"
...
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
printf 'overwatch backup round trip %s' "$STAMP" >"$TMP/probe"
...
Overwatch signing keys, backed up $STAMP.
...
updates that every Overwatch device will accept as yours.
EOF

printf '\nBacked up to %s\n' "$DEST"
printf '\nThis is still one machine. To finish:\n'
printf '  1. Encrypt it (choose your own passphrase):\n'
printf '       tar -C "%s" -czf - . | openssl enc -aes-256-cbc -pbkdf2 -out ~/overwatch-keys-%s.tar.gz.enc\n' "$DEST" "$STAMP"
...
printf '\nRestore later:\n'
printf '     openssl enc -d -aes-256-cbc -pbkdf2 -in ~/overwatch-keys-%s.tar.gz.enc | tar -xzf - -C <dir>\n' "$STAMP"
