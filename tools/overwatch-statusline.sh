#!/bin/sh
# Overwatch statusline shim.
#
# Claude Code pipes its statusline JSON to this script on every render. We keep
# a copy for the Overwatch daemon, then hand the SAME input to whatever statusline
# command was configured before Overwatch was installed, so the user's status bar
# is unchanged.
#
...
# unwritable HOME, ...) degrade silently -- Overwatch's own capture is allowed to
...
[ -d "$HOME/.overwatch" ] || mkdir -p "$HOME/.overwatch" 2>/dev/null
...
printf '%s' "$input" 2>/dev/null > "$HOME/.overwatch/statusline.json.$$.tmp" &&
  mv -f "$HOME/.overwatch/statusline.json.$$.tmp" "$HOME/.overwatch/statusline.json" 2>/dev/null

# Delegate to the previously configured command, if any. Never fail the status
# bar because Overwatch had a problem.
#
...
# Overwatch existed, so it hung identically then, and whatever timeout Claude
...
# fresh data: Overwatch keeps getting current numbers even while the user's own
# statusline is hung. Do not reorder this to "only record on success" -- that
# would let a hanging chain block Overwatch's own capture too, which is exactly
# the failure this ordering exists to prevent.
CHAIN="$HOME/.overwatch/statusline-chain"
if [ -s "$CHAIN" ]; then
  ...
  # legitimately contain an Overwatch shim invocation. In that specific case the
  ...
  if [ "$chain_arg" != "$self" ]; then
    ...
    # one message: Overwatch must never contribute terminal noise regardless of
