...
CANVAS = (320, 240)
BG = (253, 250, 241)      # the logo's own ground, sampled from its corners
...
def shapes(path):
    ...
    ink = np.abs(rgb - np.array(BG)).sum(2) > 60
...
def main():
    ...
    print("now: tools/encode_bootanim.py --frames %s --fps 12 --threshold 0 \\"
          % a.out)
    print("       --bg %d,%d,%d --out firmware/src/bootanim.h" % BG)
