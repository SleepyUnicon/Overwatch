...
```bash
. tools/lib_zephyr.sh && overwatch_zephyr_activate   # from the repo root
...
- **This builds the shipped configuration: USB only.** `CONFIG_OVERWATCH_WIFI_MODE`
...
plaintext chip (override with `OVERWATCH_SKIP_EFUSE_CHECK=1`):
...
Two secrets live **outside the repo**, in `~/.overwatch/`:

| Key | Purpose |
|-----|---------|
| `flash_key.bin` | Encrypts what's written to flash. Override with `OVERWATCH_FLASH_KEY`. |
...
```bash
# 1. bump OVERWATCH_FW_VERSION in src/version.h, commit
# 2. from the repo root:
tools/release.sh
```

`release.sh` builds from the committed tree (it refuses a dirty tree or a reused
version), signs the image, and attaches `overwatch-fw.bin` + `manifest.json` to the
`overwatch-v<version>` release on **`SleepyUnicon/Overwatch`** - which is the feed every board reads.
