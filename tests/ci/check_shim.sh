#!/bin/sh
# Exercise tools/overwatch-statusline.sh under one specific shell.
#
...
WHICH="${1:-sh}"
ci_label "$WHICH"
SHIM_SRC="$ROOT/tools/overwatch-statusline.sh"
WORK="${TMPDIR:-/tmp}/overwatch-shim-$WHICH"
...
SHIM="$HOME/overwatch-statusline.sh"
cp "$SHIM_SRC" "$SHIM"
PAYLOAD='{"rate_limits":{"five_hour":{"used_percentage":7,"resets_at":11}}}'
CAP="$HOME/.overwatch/statusline.json"
...
for t in "$HOME/.overwatch"/statusline.json.*tmp; do
...
echo "sh $HOME/their-bar.sh" >"$HOME/.overwatch/statusline-chain"
...
echo "sh $HOME/deleted-bar.sh" >"$HOME/.overwatch/statusline-chain"
...
echo "sh $SHIM" >"$HOME/.overwatch/statusline-chain"
...
printf 'bash %s\n' "$SHIM" >"$HOME/.overwatch/statusline-chain"
...
mkdir -p "$SPACED"
cp "$SHIM_SRC" "$SPACED/overwatch-statusline.sh"
printf "sh '%s/overwatch-statusline.sh'\n" "$SPACED" >"$HOME/.overwatch/statusline-chain"
printf '%s' "$PAYLOAD" | $SH "$SPACED/overwatch-statusline.sh" >/dev/null 2>"$WORK/err5.txt"
[ -s "$WORK/err5.txt" ] && fail "spaced self-chain leaked: $(cat "$WORK/err5.txt")"
ok "recognises itself at a path containing spaces"

# 7. An unwritable HOME degrades silently. Overwatch's own capture is allowed to
