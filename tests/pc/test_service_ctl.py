...
class _Runs:
    ...
    def __init__(self, codes=None, running=True):
        self.calls = []
        self.kwargs = []
        self._codes = list(codes or [])
        # What `launchctl print` says. install() and restart() ask launchd
        # rather than trusting a kickstart's exit code -- a command that was
        # accepted is not a process that exists -- so a fake that answers
        # nothing makes every launchd path report "not running".
    ...
    def __call__(self, argv, **kw):
        ...
        out = ""
        if "print" in argv:
            out = "state = running" if self._running else "state = not running"
...
def test_launchd_install_kickstarts_and_only_then_claims_it_is_running(
    ...
    # ...and then ASKS. A kickstart that returns zero is a request accepted,
...
def test_launchd_install_does_not_claim_running_when_the_kickstart_fails(
        home, monkeypatch):
    """Registered but not running must not print as running.

    The plist is on disk and the job is loaded, so the customer's next login
    does start it; what is untrue is only the word "running". And the failure
    is read off launchd's own answer rather than off the kickstart's exit
    code, which is the whole reason this path was wrong before: every command
    returned zero and nothing was running.
    """
    _platform(monkeypatch, "darwin")
    monkeypatch.setattr(cli.time, "sleep", lambda _s: None)
    r = _Runs(codes=[0, 0, 1], running=False)  # bootout, bootstrap, kickstart
    monkeypatch.setattr(cli.subprocess, "run", r)
