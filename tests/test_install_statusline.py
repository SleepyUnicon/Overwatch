...
def test_install_into_empty_settings(tmp_path, monkeypatch):
    ...
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    got = json.loads(open(path).read())
    assert got["statusLine"]["command"] == ins.statusline_command("/opt/overwatch/overwatch-statusline.sh")
...
def test_install_preserves_existing_command_in_chain(tmp_path, monkeypatch):
    ...
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    chain = (tmp_path / ".overwatch" / "statusline-chain").read_text()
...
def test_install_is_idempotent(tmp_path, monkeypatch):
    ...
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    chain = tmp_path / ".overwatch" / "statusline-chain"
    assert "overwatch-statusline.sh" not in (chain.read_text() if chain.exists() else "")
...
def test_preexisting_command_containing_marker_substring_is_preserved(tmp_path, monkeypatch):
    ...
    weird = "sh ~/scripts/wrap-overwatch-statusline.sh-backup.sh"
    path = _settings(tmp_path, {
...
def test_reinstall_at_different_shim_path_preserves_original_chain(tmp_path, monkeypatch):
    ...
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    ins.install(path, "/usr/local/bin/overwatch-statusline.sh")
    chain = (tmp_path / ".overwatch" / "statusline-chain").read_text().strip()
    assert chain == "sh ~/my-statusline.sh"
    got = json.loads(open(path).read())
    assert got["statusLine"]["command"] == ins.statusline_command("/usr/local/bin/overwatch-statusline.sh")
    ins.uninstall(path)
    got = json.loads(open(path).read())
    assert got["statusLine"]["command"] == "sh ~/my-statusline.sh"
...
def test_install_uninstall_preserves_hand_formatting(tmp_path, monkeypatch):
    ...
    ins.install(str(path), "/opt/overwatch/overwatch-statusline.sh")
    ins.uninstall(str(path))
    assert path.read_text() == original


def test_marker_lost_reinstall_at_same_path_does_not_self_chain(tmp_path, monkeypatch):
    """~/.overwatch also holds transient scratch data (statusline.json), so a
    ...
    path = _settings(tmp_path, {})
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    (tmp_path / ".overwatch" / "statusline-installed-command").unlink()

    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    chain = tmp_path / ".overwatch" / "statusline-chain"
    assert not chain.exists() or "overwatch-statusline.sh" not in chain.read_text()
...
def test_marker_lost_reinstall_at_different_path_chains_rather_than_drops(tmp_path, monkeypatch):
    ...
    ins.install(path, "/usr/local/bin/overwatch-statusline.sh")
    chain = (tmp_path / ".overwatch" / "statusline-chain").read_text().strip()
    assert chain == ins.statusline_command("/opt/overwatch/overwatch-statusline.sh")
...
def test_stale_marker_never_matches_a_customers_unrelated_command(tmp_path, monkeypatch):
    ...
    path = _settings(tmp_path, {})
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    marker = (tmp_path / ".overwatch" / "statusline-installed-command").read_text().strip()
    assert marker == ins.statusline_command("/opt/overwatch/overwatch-statusline.sh")
...
def test_uninstall_does_not_clobber_a_command_switched_to_after_install(tmp_path, monkeypatch):
    ...
    assert "leaving it alone" in msg.lower() or "isn't overwatch" in msg.lower()
...
def test_uninstall_never_installed_leaves_unrelated_command_untouched(tmp_path, monkeypatch):
    ...
    someone else's command, and Overwatch never installed here (no marker, no
    chain file -- e.g. ~/.overwatch was wiped, or this machine never ran
...
def test_uninstall_after_overwatch_directory_wiped_leaves_statusline_alone(tmp_path, monkeypatch):
    """~/.overwatch (marker + chain) gone entirely, but settings.json still
    names the Overwatch shim as statusLine (a real, currently-active install).
    ...
    import shutil
    shutil.rmtree(tmp_path / ".overwatch")
...
def test_uninstall_with_shim_path_recognizes_ours_even_without_marker(tmp_path, monkeypatch):
    ...
    already does -- so losing just the marker (not all of ~/.overwatch) still
    ...
    ins.uninstall(path, "/opt/overwatch/overwatch-statusline.sh")
    got = json.loads(open(path).read())
...
def test_install_quotes_shim_path_containing_a_space(tmp_path, monkeypatch):
    path = _settings(tmp_path, {})
    spaced = "/Users/kfir/Application Support/overwatch/overwatch-statusline.sh"
...
def test_install_with_no_previous_statusline_clears_a_ghost_chain_file(tmp_path, monkeypatch):
    ...
    path = _settings(tmp_path, {})
    overwatch_dir = tmp_path / ".overwatch"
    overwatch_dir.mkdir(parents=True)
    (overwatch_dir / "statusline-chain").write_text("sh ~/ghost-command.sh\n")

    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    chain = overwatch_dir / "statusline-chain"
...
def test_install_with_marker_but_no_statusline_preserves_a_live_chain(tmp_path, monkeypatch):
    ...
    not report 'Removed the Overwatch statusline' with nothing to restore."""
    path = _settings(tmp_path, {
        "statusLine": {"type": "command", "command": "sh ~/original.sh"}
    })
    ins.install(path, "/opt/overwatch/overwatch-statusline.sh")
    chain_path = tmp_path / ".overwatch" / "statusline-chain"
...
def test_windows_writes_bash_not_sh(monkeypatch):
    ...
    so `sh C:/x/overwatch-statusline.sh` becomes `bash sh C:/x/...`, and bash
    ...
    got = ins.statusline_command(r"C:\Users\kfir\.overwatch\overwatch-statusline.sh")
    assert got == "bash C:/Users/kfir/.overwatch/overwatch-statusline.sh"


def test_posix_still_writes_sh(monkeypatch):
    monkeypatch.setattr(ins.sys, "platform", "darwin")
    assert ins.statusline_command("/opt/overwatch/overwatch-statusline.sh") == \
        "sh /opt/overwatch/overwatch-statusline.sh"
